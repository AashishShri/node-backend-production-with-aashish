import joi from 'joi'
import { IChangePasswordRequestBody, IForgotPasswordRequestBody, ILoginUserRequestBody, IRegisterUserRequestBody, IResetPasswordRequestBody } from '../types/userTypes'
...
export const ValidateChangePasswordBody = joi.object<IChangePasswordRequestBody, true>({
    ...
    newPassword: joi.string().min(8).max(24).trim().required(),
    confirmPassword: joi.string().min(8).max(24).trim().valid(joi.ref('newPassword')).required()
})
